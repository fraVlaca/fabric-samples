...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset CreateAsset(final Context ctx, final String assetID, final String color, final int size, final int appraisedValue) {
        ChaincodeStub stub = ctx.getStub();
        boolean err = ctx.getClientIdentity().assertAttributeValue("abac.creator", "true");
        if(!err){
            String errorMessage = "Client not autorized, set abac.creator=true";
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.CLIENT_NOT_AUTHORIZED.toString());
        }
	    ...
	    String clientID = this.GetSubmittingClientIdentity(ctx);

        Asset asset = new Asset(assetID, color, size, clientID, appraisedValue);
     ...
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public Asset UpdateAsset(final Context ctx, final String assetID, final String color, final int size, final int appraisedValue) {
        ChaincodeStub stub = ctx.getStub();
        ...
        Asset newAsset = new Asset(assetID, color, size, clientID, appraisedValue);
    ...
    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetSubmittingClientIdentity(final Context ctx) {

        String b64ID = ctx.getClientIdentity().getId();

        if (b64ID == null){
            String errorMessage = "failed to retrieve Client ID";
            throw new ChaincodeException(errorMessage);
        }

        byte[] decodeID = Base64.getDecoder().decode(b64ID);
        
        return new String(decodeID);
    }
}
